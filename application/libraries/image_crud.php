...
class image_CRUD {
	...
	protected $default_language_path = 'plugins/js/image_crud/image_crud/languages';
	...
	function set_draganddrop($draganddrop)
	{
	...
	function set_relation_value($field_value)
	{
		$this->relation_value = $field_value;
	...
	protected function _library_view($view, $vars = array(), $return = FALSE)
	{
		...
		$view_file = 'plugins/js/image_crud/image_crud/views/';
    ...
    protected function _delete_files($relation)
    {
    	$this->ci->db->where($this->relation_field,$relation);
    	$results = $this->ci->db->get($this->table_name)->result();
    	foreach ($results as $row) {
    		unlink( $this->image_path.'/'.$row->{$this->url_field} );
    		unlink( $this->image_path.'/'.$this->thumbnail_prefix.$row->{$this->url_field} );

    		$this->ci->db->delete($this->table_name, array($this->primary_key => $row->{$this->primary_key}));
    	}
    ...
    protected function _get_delete_urls($value)
    {
    	$rsegments_array = $this->ci->uri->rsegment_array();
    	return site_url($rsegments_array[1].'/'.$rsegments_array[2].'/delete_files/'.$value);
    }
    ...
    protected function _get_photos($relation_value = null)
    {
    	...
    	foreach($results as $num => $row)
    	{
    		...
    		// if(!empty($relation_value))
	    	// {
	    		// $results[$num]->delete_url = $this->_get_delete_urls($row->{$this->primary_key});
	    	// } else {
	...
	{
		include('plugins/js/image_crud/image_crud/config/translit_chars.php');
	...
	protected function getState()
	{
		...
		// 	return (object)$state;
		// }
		elseif( (empty($rsegments_array[3]) && !empty($this->relation_field) && !empty($this->relation_value)) || (!empty($rsegments_array[3]) &&  $rsegments_array[3] == 'ajax_list'))
		{
			$upload_url = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/upload_file');
			$ajax_list_url  = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/ajax_list');
			$ordering_url  = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/ordering');
			...
			$state = array( 'name' => 'list', 'upload_url' => $upload_url, 'relation_value' => $this->relation_value);
			$state['ajax'] = isset($rsegments_array[3]) && $rsegments_array[3] == 'ajax_list'  ? true : false;
			$state['ajax_list_url'] = $ajax_list_url;
			$state['ordering_url'] = $ordering_url;
			$state['insert_title_url'] = $insert_title_url;
		...
		elseif(isset($rsegments_array[3]) && $rsegments_array[3] == 'upload_file')
		{
			...
			if(!empty($this->relation_value)) {
				$results['relation_value'] = $this->relation_value;
			}
		...
		elseif(isset($rsegments_array[3]) && isset($rsegments_array[4]) && $rsegments_array[3] == 'delete_files')
		{
			$state = array( 'name' => 'delete_files', 'relation' => $this->relation_value);
			return (object)$state;
		}
		...
		elseif( (empty($rsegments_array[4]) && !empty($this->relation_field) && !empty($this->relation_value)) || (!empty($rsegments_array[4]) &&  $rsegments_array[4] == 'ajax_list'))
		{
			...
			switch ($state_info->name) {
				case 'list':
					...
					$this->_library_view('list.php',array(
						...
						'can_draganddrop' => $this->draganddrop
					));
				...
				case 'delete_files':
					...
					$relation = $state_info->relation;

					$this->_delete_files($relation);
